...
export class UserStore {
    ...
    async create(u: User): Promise<User> {
        const hash = bcrypt.hashSync(u.password + HASH_SECRET, parseInt(SALT_ROUNDS))
    ...
    async update(id: User["id"], u: User): Promise<User> {
        const conn = await client.connect()
        const { firstName, password } = await this.show(id)
        try {
            const hash = bcrypt.hashSync(u?.password + HASH_SECRET, parseInt(SALT_ROUNDS)) || null
            const sql = 'UPDATE users Set name = $1 WHERE id = $3 RETURNING *'
            const result = await conn
                .query(sql, [u.firstName || firstName, hash || password, id])
            const user = result.rows[0]
    ...
    async authenticate(username: User["username"], password: User["password"]): Promise<User | null> {
